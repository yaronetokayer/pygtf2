...
@njit(
    types.Tuple((float64[:, :], float64[:], float64[:]))(
        float64[:, :],  # r_edges: (s, N+1) -- shared geometry, identical rows
        float64[:, :],  # rho:     (s, N)
        float64[:, :],  # v2:      (s, N)
        float64,        # c_d:     scalar tunable prefactor
        types.boolean,  # use_harmonic_mix
    ),
    cache=True, fastmath=True
)
def compute_f_fick(r_edges, rho, v2, c_d, use_harmonic_mix) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """
    Compute species-wise Fick flux at interior interfaces with zero-net-flux projection.

    Returns
    -------
    F_if      : (s, N-1)  species flux at interior interfaces (outward-positive)
    dr_if     : (N-1,)    center-to-center spacing used for gradients
    ...
    for i in range(N-1):
        rt = 0.0
        for k in range(s):
            rho_if[k, i] = 0.5 * (rho[k, i] + rho[k, i+1])
            if rho_if[k, i] < tiny:
                rho_if[k, i] = tiny
            v2_if[k, i]  = 0.5 * (v2[k, i]  + v2[k, i+1])
            if v2_if[k, i] < tiny:
                v2_if[k, i] = tiny
    ...
    # D_k at interfaces: D_k ≈ c_d * sqrt(v2_k) / rho_k
