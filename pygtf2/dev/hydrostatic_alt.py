...
### METHODS FROM WHEN REVIRIALIZE DID NOT DO IN-PLACE UPDATES, AND WAS NOT JITTED ###

def revirialize_interp(r, rho, p, m, bkg_param) -> tuple[
    str, np.ndarray | None, np.ndarray | None, np.ndarray | None, float | None, float | None
]:
    """
    Multi-species re-virialization.
    Solves for radius adjustments and updates physical quantities for all species.
...
def solve_tridiagonal_thomas(a, b, c, y) -> np.ndarray:
    ...
    For an N×N system, all coefficient arrays have length N.

    Arguments
    ---------
    a : ndarray
        Subdiagonal coefficients. a[i] multiplies x[i-1].
        The value a[0] is unused.
    b : ndarray
        Main diagonal coefficients. b[i] multiplies x[i].
    c : ndarray
        Superdiagonal coefficients. c[i] multiplies x[i+1].
        The value c[N-1] is unused.
    y : ndarray
